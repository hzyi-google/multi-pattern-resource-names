...
namespace Google.Devtools.Containeranalysis.V1beta1
{
    ...
    /// <summary>
    /// Resource name for the 'scan_config' resource.
    /// </summary>
    public sealed partial class ScanConfigName : gax::IResourceName, sys::IEquatable<ScanConfigName>
    {
        private static readonly gax::PathTemplate s_template = new gax::PathTemplate("projects/{project}/scanConfigs/{scan_config}");

        /// <summary>
        /// Parses the given scan_config resource name in string form into a new
        /// <see cref="ScanConfigName"/> instance.
        /// </summary>
        /// <param name="scanConfigName">The scan_config resource name in string form. Must not be <c>null</c>.</param>
        /// <returns>The parsed <see cref="ScanConfigName"/> if successful.</returns>
        public static ScanConfigName Parse(string scanConfigName)
        {
            gax::GaxPreconditions.CheckNotNull(scanConfigName, nameof(scanConfigName));
            gax::TemplatedResourceName resourceName = s_template.ParseName(scanConfigName);
            return new ScanConfigName(resourceName[0], resourceName[1]);
        }

        /// <summary>
        /// Tries to parse the given scan_config resource name in string form into a new
        /// <see cref="ScanConfigName"/> instance.
        /// </summary>
        /// <remarks>
        /// This method still throws <see cref="sys::ArgumentNullException"/> if <paramref name="scanConfigName"/> is null,
        /// as this would usually indicate a programming error rather than a data error.
        /// </remarks>
        /// <param name="scanConfigName">The scan_config resource name in string form. Must not be <c>null</c>.</param>
        /// <param name="result">When this method returns, the parsed <see cref="ScanConfigName"/>,
        /// or <c>null</c> if parsing fails.</param>
        /// <returns><c>true</c> if the name was parsed successfully; <c>false</c> otherwise.</returns>
        public static bool TryParse(string scanConfigName, out ScanConfigName result)
        {
            gax::GaxPreconditions.CheckNotNull(scanConfigName, nameof(scanConfigName));
            gax::TemplatedResourceName resourceName;
            if (s_template.TryParseName(scanConfigName, out resourceName))
            {
                result = new ScanConfigName(resourceName[0], resourceName[1]);
                return true;
            }
            else
            {
        ...
        /// <summary>Formats the IDs into the string representation of the <see cref="ScanConfigName"/>.</summary>
        /// <param name="projectId">The <c>project</c> ID. Must not be <c>null</c>.</param>
        /// <param name="scanConfigId">The <c>scanConfig</c> ID. Must not be <c>null</c>.</param>
        /// <returns>The string representation of the <see cref="ScanConfigName"/>.</returns>
        public static string Format(string projectId, string scanConfigId) =>
            s_template.Expand(gax::GaxPreconditions.CheckNotNull(projectId, nameof(projectId)), gax::GaxPreconditions.CheckNotNull(scanConfigId, nameof(scanConfigId)));

        /// <summary>
        /// Constructs a new instance of the <see cref="ScanConfigName"/> resource name class
        /// from its component parts.
        /// </summary>
        /// <param name="projectId">The project ID. Must not be <c>null</c>.</param>
        /// <param name="scanConfigId">The scanConfig ID. Must not be <c>null</c>.</param>
        public ScanConfigName(string projectId, string scanConfigId)
        {
            ProjectId = gax::GaxPreconditions.CheckNotNull(projectId, nameof(projectId));
            ScanConfigId = gax::GaxPreconditions.CheckNotNull(scanConfigId, nameof(scanConfigId));
        }
        ...
        /// <summary>
        /// The scanConfig ID. Never <c>null</c>.
        /// </summary>
        ...
        /// <inheritdoc />
        public override string ToString() => s_template.Expand(ProjectId, ScanConfigId);
